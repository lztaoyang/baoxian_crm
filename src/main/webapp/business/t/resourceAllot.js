	...
	var vuePage = new Vue({
		...
		methods: {
			...
			readList: function() {
				...
				this.$http.put(this.sourceUrl + '/read/userAllotNum')
					.then(function(response) {
			...
			readDetail: function(entityId) {
				this.$http.put(this.sourceUrl + '/read/detail', {
						id: entityId
					})
					.then(function(response) {
			...
			del: function(entityIds) {
				...
				self.$confirm('您确定要删除' + params.ids.length + '条记录吗?, 删除将无法恢复!', '提示', {
					...
					self.$http.delete(self.sourceUrl, {
							body: params
						})
						.then(function(response) {
							var tempIndexArray = [];
			...
			readAllUser: function() {
				...
				this.$http.put('sys/dept/read/list', {
						"keyword": '市场',
						"enable": "1",
						"pageSize": "100"
					})
			...
			treeNodeClick: function(selected) {
				this.$http.put("sys/user/read/list", {
						"deptId": selected.id,
						"enable": "1",
						"locked": "0",
						"pageSize": "100"
			...
			allot: function() {
				...
				if(self.allotRow.length == 0) {
					...
					self.$confirm('您确定要调配给[' + self.allotRow.name + ']吗?', '提示', {
						...
						self.$http.post(self.allotUrl, {
								"ids": params.ids,
								"userId": self.allotRow.id
							})
							.then(function(response) {
            ...
            exportAllotDetail : function () {
            	...
            	window.open(encodeURI('/t/resourceAllot/exportAllotDetail?' + str));
            }
